...
function searchRoutine() {
    ...
    for(let i=0; i<rawExcelData.length; i++) {
        for(let j=0; j<rawExcelData[i].length; j++) {
            ...
            // 🎯 THE MAGIC FIX: RegEx দিয়ে একদম সলিড ব্যাচ নাম্বার খোঁজা হচ্ছে
            // যাতে 1206 কোর্সের 06 আর 73 ব্যাচের 7 মিলে 67 হয়ে না যায়!
            let hasBatch = true;
            if (batch !== "") {
                // এই লজিকটা চেক করবে ব্যাচ নাম্বারের আগে বা পরে যেন অন্য কোনো সংখ্যা না থাকে
                let batchRegex = new RegExp("(^|[^0-9])" + batch + "([^0-9]|$)");
                hasBatch = batchRegex.test(cellUpper);
            }
            if (!hasBatch) continue;

            // 🎯 অ্যাডভান্সড সেকশন লজিক (কম্বাইন্ড ব্যাচের জন্য)
            // 🎯 THE MAGIC FIX: সেকশন চেক করার নিঞ্জা লজিক!
            let hasSection = true;
            if (section !== "") {
                // এই লজিকটা খুঁজবে: ব্যাচ, ব্র্যাকেট বা SEC-এর পর সেকশন লেটার আছে কি না।
                // (?![A-Z]) অংশটা নিশ্চিত করবে যে, সেকশন লেটারের পর যেন আর কোনো অক্ষর না থাকে 
                // (যাতে 'A' খুঁজলে সেটা টিচার 'ARK' বা 'AT'-এর 'A' হিসেবে ম্যাচ না করে যায়!)
                let secRegex = new RegExp("(" + batch + "|\\(|\\)|SEC[-:]*)" + section + "(?![A-Z])", "i");
                
                // যদি এক্সেলে শুধু স্পেস দিয়ে আলাদাভাবে সেকশন লেখা থাকে (যেমন: " 69 A " বা শেষে " A")
